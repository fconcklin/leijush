...
(defn runRound
  ...
  (let [z (genetic-runner (first players) (second tournament) (first (first tournament)) cap)
        x (if (= z 0) 0 1)]
    (cons x
     (pmap
      (fn [x y]
	(runChoose x(second tournament) y cap))
      (rest players)
...
(defn getScore [payOffs]
  (pmap
   (fn [x]
     (apply + x))
   payOffs))

(defn run				; error-threshold 40
  [error strat]
  (pushgp
...
(run 40 strategy2)			; error-threshold
